<template>
    <div class="edit-futsal-page">
        <div class="container">
            <div class="page-header">
                <h1>Edit Futsal</h1>
                <p>Update the details below to modify your futsal listing on Booksall</p>
            </div>
            ...
            <div class="form-container" v-if="!isLoading">
                <form @submit.prevent="submitForm">
                    ...
                    <div v-if="currentStep === 0" class="form-step">
                        ...
                        <p>Update the essential details about your futsal</p>
                    ...
                    <div v-if="currentStep === 1" class="form-step">
                        ...
                        <p>Update the facilities and features of your futsal</p>
                    ...
                    <div v-if="currentStep === 2" class="form-step">
                        <h2>Upload Images</h2>
                        <p>Update up to 5 high-quality images of your futsal</p>

                        <div class="image-upload-container">
                            <div v-for="n in 5" :key="n" class="image-upload-box"
                                :class="{ 'has-image': formData[`futsal_image_${n}`] || previewImages[n - 1] }">
                                <div v-if="formData[`futsal_image_${n}`] || previewImages[n - 1]" class="image-preview">
                                    <img :src="previewImages[n - 1] || formData[`futsal_image_${n}`]"
                                        :alt="`Futsal Image ${n}`" />
                    ...
                    <div v-if="currentStep === 3" class="form-step">
                        <h2>Price Table</h2>
                        <p>Update different pricing options for your futsal</p>
                    ...
                    <div v-if="currentStep === 4" class="form-step">
                        <h2>Description</h2>
                        <p>Update the detailed description of your futsal</p>
            ...
            <div v-else class="loading">Loading futsal data...</div>
        </div>
    </div>
</template>
...
const fetchFutsalData = async () => {
    ...
    try {
        ...
        Object.assign(formData, {
            ...
            futsal_phone_number: String(data.futsal_phone_number),
            futsal_image_1: data.futsal_image_1,
            futsal_image_2: data.futsal_image_2,
            futsal_image_3: data.futsal_image_3,
            futsal_image_4: data.futsal_image_4,
            futsal_image_5: data.futsal_image_5,
            price_per_hour: data.price_per_hour,
            price_table: data.price_table,
            location: data.location,
            google_maps_link: data.google_maps_link,
            a_side: String(data.a_side),
            grounds: data.grounds,
            shower_facility: data.shower_facility,
            parking_space: data.parking_space,
            changing_room: data.changing_room,
            restaurant: data.restaurant,
            wifi: data.wifi,
            open_ground: data.open_ground,
            futsal_description: data.futsal_description,
            featured_futsal: data.featured_futsal,
            futsal_owner: data.futsal_owner
        });

        // Populate preview images
        for (let i = 1; i <= 5; i++) {
            if (data[`futsal_image_${i}`]) {
                previewImages.value[i - 1] = data[`futsal_image_${i}`];
            }
        }

        // Populate price table
        if (data.price_table) {
            priceTable.value = Object.entries(data.price_table).map(([name, value]) => ({
                name,
                value: String(value)
            }));
        } else {
            priceTable.value = [{ name: '', value: '' }];
        }

        // Set editor content
        if (editor.value) {
            editor.value.innerHTML = data.futsal_description;
        }
    } catch (err) {
        console.error('Error fetching futsal data:', err);
        errors.api_error = 'Failed to load futsal data. Please try again.';
        ...
        if (currentStep.value === 3) {
            ...
            priceTable.value.forEach(price => {
                if (price.name && price.value) {
                    priceObj[price.name] = Number(price.value);
                }
            });
...
const submitForm = async () => {
    ...
    try {
        ...
        for (let i = 1; i <= 5; i++) {
            if (formData[`futsal_image_${i}`] instanceof File) {
        ...
        const response = await apiClient.patch(`futsal/update-futsal/${formData.id}/`, submitData, {
        ...
        errors.api_error = error.response?.data?.detail || 'An error occurred while updating the futsal. Please try again.';
    } finally {
        isSubmitting.value = false;
    }
};

// Initialize editor and fetch futsal data
onMounted(async () => {
    await initialize();
});
</script>
...
/* Use the same container, page-header, form-progress, etc. styles as before */
.loading {
    text-align: center;
    font-size: 1.2rem;
    color: #666;
